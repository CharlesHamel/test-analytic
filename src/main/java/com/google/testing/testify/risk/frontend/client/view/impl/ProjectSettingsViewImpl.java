...
public class ProjectSettingsViewImpl extends Composite implements ProjectSettingsView {
  ...
  protected void onUpdateProjectInfoButtonClicked(ClickEvent event) {
    ...
    if (presenter != null) {
      if (deleteProjectCheckBox.getValue()) {
        ...
        if (newOwners.size() < 1) {
          Window.alert("Erreur: le projet doit avoir au moins un propriétaire.");
          return;
        }
        ...
        if (difference.size() > 0) {
          warning.append("<br><br><b>Propriétaires ajoutés:</b> ");
        ...
        if (warning.length() > 0) {
          ...
          box.setTitle("Changements de permissions");
          box.add(new HTML("Vous modifiez certaines des autorisations pour ce projet."
  ...
  void onDeleteProjectCheckBoxChecked(ClickEvent event) {
    String warningMessage = "Cela supprimera définitivement votre projet lorsque vous cliquez sur Enregistrer."
      + " Êtes-vous sûr?";

...
public class HomePage extends Composite {
  ...
  private void loadProjectList() {
    userService.getStarredProjects(
        new TaCallback<List<Long>>("recherche des projets favoris") {
          @Override
    ...
    displayMessageInGrid("Chargement de la liste des projetss...");
  ...
  private void doCreateProject(String projectName) {
    ...
    if (projectName.length() < 1) {
      Window.alert("Saisissez un nom de projet valide.");
      return;
    }
    ...
    projectService.createProject(newProject,
        new TaCallback<Long>("création d'un nouveau projet") {
          @Override
  ...
  void onCreateTestProjectButtonClicked(ClickEvent event) {
    ...
    creatorService.createTestProject(
        new TaCallback<Project>("création d'un nouveau projet") {
          @Override
  ...
  private void updateProjectsList() {
    if (allProjects != null && starredProjects != null) {
      ...
      if (userProjects.size() < 1) {
        displayMessageInGrid("Aucun projet à afficher.");
        return;
      }

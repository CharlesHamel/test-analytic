...
public class CapabilityDetailsViewImpl extends Composite implements CapabilityDetailsView {
  ...
  private Widget buildTestHeaderWidget(String header, String addText) {
    ...
    Button button = new Button(" Ajouter ", new ClickHandler() {
        @Override
  ...
  private void updateTestSection() {
    ...
    testGrid.setWidget(0, 0, buildTestHeaderWidget("Activité de test récente", "ajouter test"));
    ...
    for (int i = 0; i < capabilityTests.size(); i++) {
      ...
      if (state < 0) {
        statusLabel.setText(" - échoué " + getDateText(test.getStateDate()));
        failed++;
      } else if (state > 0) {
        statusLabel.setText(" - réussit " + getDateText(test.getStateDate()));
        passed++;
      } else {
        statusLabel.setText(" - aucun résultat");
        notRun++;
      }
      panel.add(statusLabel);
      testGrid.setWidget(i + 1, 0, panel);
    }
    testNotRunCount.setHTML("pas exécuté <b>" + notRun + "</b>");
    testPassedCount.setHTML("réussit <b>" + passed + "</b>");
    testFailedCount.setHTML("échoué <b>" + failed + "</b>");
  ...
  private void updateBugSection() {
    ...
    for (int i = 0; i < capabilityBugs.size(); i++) {
      ...
      panel.add(anchor);
      Label statusLabel = new Label();
      statusLabel.setText(" - remplit " + getDateText(bug.getStateDate()));
      panel.add(statusLabel);
  ...
  private void updateCheckinsSection() {
    changeGrid.setTitle("Modifications récentes du code (" + capabilityCheckins.size() + " total)");
    changeGrid.resize(capabilityCheckins.size() + 1, 1);
    changeGrid.setWidget(0, 0, buildCheckinHeaderWidget(
        "Modifications récentes du code (" + capabilityCheckins.size() + " total)", "ajouter du changement au code"));
    ...
    for (int i = 0; i < capabilityCheckins.size(); i++) {
      ...
      panel.add(anchor);
      Label statusLabel = new Label();
      statusLabel.setText(" - envoyé " + getDateText(checkin.getStateDate()));
      panel.add(statusLabel);

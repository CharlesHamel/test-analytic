...
public class AttributeViewImpl extends Composite implements AttributeView {
  ...
  public AttributeViewImpl() {
    ...
    description.getElement().setAttribute("placeholder", "Entrez la description de cet attribut...");
  }
  ...
  protected void onAttributeNameChanged(ValueChangeEvent<String> event) {
    if (presenter != null) {
      if (event.getValue().length() == 0) {
        NotificationUtil.displayErrorMessage("Nom non valide pour l'attribut.");
  ...
  protected void onDeleteAttributeImageClicked(ClickEvent event) {
    String promptText = "Voulez-vous vraiment supprimer " + attributeName.getText() + "?";

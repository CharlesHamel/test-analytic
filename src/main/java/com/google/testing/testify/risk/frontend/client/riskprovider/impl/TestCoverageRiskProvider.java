...
public class TestCoverageRiskProvider implements RiskProvider {
  ...
  public void initialize(List<CapabilityIntersectionData> projectData) {
    ...
    dataService.getProjectTestCasesById(projectId,
        new TaCallback<List<TestCase>>("Recherche des cas de test") {
          @Override
    ...
    for (TestCase test : testCases) {
      for (String testcaseTag : test.getTags()) {
        ...
        try {
          ...
          errorMessage.append("Erreur lors du traitement d'un cas de test avec une balise mal formée. ");
          errorMessage.append("Balise de cas de test: '");
          errorMessage.append(testcaseTag);
          errorMessage.append("' trouvé sur le cas de test '");
  ...
  public Widget onClick(CapabilityIntersectionData targetCell) {
    ...
    if (testCases.size() == 0) {
      panel.add(new Label("Aucun cas de test n'est associé à cette cellule."));
    } else {

...
public class ComponentViewImpl extends Composite implements ComponentView {
  ...
  public ComponentViewImpl() {
    ...
    description.getElement().setAttribute("placeholder", "Entrez la description de ce composant...");
  }
  ...
  protected void onComponentNameChanged(ValueChangeEvent<String> event) {
    if (presenter != null) {
      if (event.getValue().length() == 0) {
        NotificationUtil.displayErrorMessage("Nom non valide pour le Composant.");
  ...
  protected void onDeleteComponentImageClicked(ClickEvent event) {
    String promptText = "Voulez-vous vraiment supprimer " + componentName.getText() + "?";
  ...
  private void addBlankLabel() {
    final String newText = "Ajouter label";
